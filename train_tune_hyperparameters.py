...
def train(args):
    ...
    if args.dataset =="ade20k":
        t_loader = data_loader(data_path, is_transform=True, img_size=(args.img_rows, args.img_cols),
                               augmentations=Compose([RandomHorizontallyFlip()]))
    ...
    # Setup Model
    model = get_model(args.arch, n_classes, version="pascalvoc")
    ...
    if hasattr(model.module, 'optimizer'):
        ...
        #optimizer = torch.optim.(model.parameters(), lr=args.l_rate, momentum=0.99, weight_decay=5e-4)
        #optimizer = torch.optim.Adam(model.parameters(), lr=args.l_rate)
        optimizer = torch.optim.SGD(model.parameters(), lr=args.l_rate, momentum=0.99, weight_decay=5e-4)
    if args.exp_decay:
        print("Use exp_decay, decay epoch:",args.exp_decay)
        print("Decrease by ", args.gamma)
        scheduler = sch.StepLR(optimizer,args.exp_decay,gamma=0.1)
    ...
    for epoch in range(args.n_epoch):
        ...
        np.savetxt( 'log_miou.txt', np.array(loss_miou))
        np.savetxt('log_acc.txt',np.array(loss_acc))
        np.savetxt('log_loss.txt',np.array(loss_loss))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_decay', nargs='?', type=int,
                        help='use exp_decay')
